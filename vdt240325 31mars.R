...
diversity_indices <- data3 %>% 
  group_by(Sites,Pratiques) %>%
  summarise(
    Pielou = diversity(table(Binomial), index="shannon")/log(n_distinct(Binomial)),
    Simpson = diversity(table(Binomial), index="simpson")
    Hill = exp(diversity(table(Binomial), index=("shannon")), 
    .groups="drop" 
  )
    
#1 Tests pour Piélou 
myAOV_pielou = aov(Pielou ~ Pratiques, data=diversity_indices)
summary(myAOV_pielou)
#Vérification de la normalité 
hist(residuals(myAOV_pielou))
qqnorm(resoduals(myAOV_pielou))
qqline(resoduals(myAOV_pielou))
shapiro.test(residuals(myAOV_pielou))
#Vérification de l'homoscédasticité 
bartlett.test(Pielou ~ Pratiques, data=diversity_indices)
#si condi. d'appli. non respectées -> version non para. 
kruskal.test(Pielou ~ Pratiques, data=diversity_indices)
#Graphique 
ggplot(diversity_indices, aes(x=Pratiques, y=Piélou, fill=Pratiques))+
  geom_boxplot(outlier.shape=NA, alpha=0.7)+
  geom_jitter(width=0.2, alpha=0.5, color="black")+
  scale_fill_manual(values=palette_colors)+
...
#2 Tests pour Simpson 
myAOV_simpson = aov(Simpson ~ Pratiques, data=diversity_indices)
summary(myAOV_simpson)
#Vérification de la normalité 
hist(residuals(myAOV_simpson))
qqnorm(resoduals(myAOV_simpson))
qqline(resoduals(myAOV_simpson))
shapiro.test(residuals(myAOV_simpson))
#Vérification de l'homoscédasticité 
bartlett.test(Simpson ~ Pratiques, data=diversity_indices)
#si condi. d'appli. non respectées -> version non para. 
kruskal.test(Simpson ~ Pratiques, data=diversity_indices)
#Graphique 
ggplot(diversity_indices, aes(x=Pratiques, y=Simpson, fill=Pratiques))+
  geom_boxplot(outlier.shape=NA, alpha=0.7)+
  geom_jitter(width=0.2, alpha=0.5, color="black")+
  scale_fill_manual(values=palette_colors)+
  labs(title="Indice de Piélou de vers de terre par pratique agricole",
...
#3 Tests pour Hill 
myAOV_hill = aov(Hill ~ Pratiques, data=diversity_indices)
summary(myAOV_hill)
#Vérification de la normalité 
hist(residuals(myAOV_hill))
qqnorm(resoduals(myAOV_hill))
qqline(resoduals(myAOV_hill))
shapiro.test(residuals(myAOV_hill))
#Vérification de l'homoscédasticité 
bartlett.test(Hill ~ Pratiques, data=diversity_indices)
#si condi. d'appli. non respectées -> version non para. 
kruskal.test(Hill ~ Pratiques, data=diversity_indices)
#Graphique 
ggplot(diversity_indices, aes(x=Pratiques, y=Hill, fill=Pratiques))+
  geom_boxplot(outlier.shape=NA, alpha=0.7)+
  geom_jitter(width=0.2, alpha=0.5, color="black")+
  scale_fill_manual(values=palette_colors)+
