...
#Validité du modèle (pas nécessaire ici car aucune régression n'est significative)

#Diagnostic graphique
par(mfrow=c(2,2))
plot(reg_moyenne_3ans)
#Diagnostic numérique
#Présence d'outliers parmi les résidus studentisés
outlierTest(reg_moyenne_3ans) #p-value de Bonferonni doit être supérieure à 0,05
#Normalité des résidus
jarque.bera.test(reg_moyenne_3ans$res) #s'interprète comme Shapiro
#Homoscédasticité des résidus
bptest(reg_moyenne_3ans) #p-value doit être supérieure à 0,05
#Indépendance des résidus avec test auto-corrélation de Durbin-Watson
durbinWatsonTest(reg_moyenne_3ans) #D-W doit être proche de 2 et p-value > 0,05
...
#Validité du modèle
outlierTest(reg_biom_sum_2ans) #p-value de Bonferonni doit être supérieure à 0,05
jarque.bera.test(reg_biom_sum_2ans$res) #s'interprète comme Shapiro
bptest(reg_biom_sum_2ans) #p-value doit être supérieure à 0,05
durbinWatsonTest(reg_biom_sum_2ans) #D-W doit être proche de 2 et p-value > 0,05

outlierTest(reg_biom_sum_1an) #p-value de Bonferonni doit être supérieure à 0,05
jarque.bera.test(reg_biom_sum_1an$res) #s'interprète comme Shapiro
bptest(reg_biom_sum_1an) #p-value doit être supérieure à 0,05
durbinWatsonTest(reg_biom_sum_1an) #D-W doit être proche de 2 et p-value > 0,05
...
#Validité du modèle
outlierTest(reg_esp_moyenne_3ans)
jarque.bera.test(reg_esp_moyenne_3ans$residuals)
bptest(reg_esp_moyenne_3ans)
durbinWatsonTest(reg_esp_moyenne_3ans)

outlierTest(reg_esp_moyenne_2ans)
jarque.bera.test(reg_esp_moyenne_2ans$residuals)
bptest(reg_esp_moyenne_2ans)
durbinWatsonTest(reg_esp_moyenne_2ans)

outlierTest(reg_esp_sum_4ans)
jarque.bera.test(reg_esp_sum_4ans$residuals)
bptest(reg_esp_sum_4ans)
durbinWatsonTest(reg_esp_sum_4ans)

outlierTest(reg_esp_sum_3ans)
jarque.bera.test(reg_esp_sum_3ans$residuals)
bptest(reg_esp_sum_3ans)
durbinWatsonTest(reg_esp_sum_3ans)

outlierTest(reg_esp_sum_2ans)
jarque.bera.test(reg_esp_sum_2ans$residuals)
bptest(reg_esp_sum_2ans)
durbinWatsonTest(reg_esp_sum_2ans)
...
stir_aggrege_shannon <- left_join(stir_aggrege, shannon_df, by = "site")

# Régressions linéaires pour l'indice de Shannon
reg_shannon_moyenne_3ans <- lm(Shannon ~ STIR_moy_3ans, data = stir_aggrege_shannon)
summary(reg_shannon_moyenne_3ans)
reg_shannon_moyenne_2ans <- lm(Shannon ~ STIR_moy_2ans, data = stir_aggrege_shannon)
summary(reg_shannon_moyenne_2ans)
reg_shannon_sum_4ans <- lm(Shannon ~ STIR_sum_4ans, data = stir_aggrege_shannon)
summary(reg_shannon_sum_4ans)
reg_shannon_sum_3ans <- lm(Shannon ~ STIR_sum_3ans, data = stir_aggrege_shannon)
summary(reg_shannon_sum_3ans)
reg_shannon_sum_2ans <- lm(Shannon ~ STIR_sum_2ans, data = stir_aggrege_shannon)
summary(reg_shannon_sum_2ans)
reg_shannon_sum_1an <- lm(Shannon ~ STIR_sum_1an, data = stir_aggrege_shannon)
summary(reg_shannon_sum_1an)

#Validité du modèle

# Validité des modèles pour Shannon
outlierTest(reg_shannon_moyenne_3ans)
jarque.bera.test(reg_shannon_moyenne_3ans$residuals)
bptest(reg_shannon_moyenne_3ans)
durbinWatsonTest(reg_shannon_moyenne_3ans)

outlierTest(reg_shannon_moyenne_2ans)
jarque.bera.test(reg_shannon_moyenne_2ans$residuals)
bptest(reg_shannon_moyenne_2ans)
durbinWatsonTest(reg_shannon_moyenne_2ans)

outlierTest(reg_shannon_sum_4ans)
jarque.bera.test(reg_shannon_sum_4ans$residuals)
bptest(reg_shannon_sum_4ans)
durbinWatsonTest(reg_shannon_sum_4ans)

outlierTest(reg_shannon_sum_3ans)
jarque.bera.test(reg_shannon_sum_3ans$residuals)
bptest(reg_shannon_sum_3ans)
durbinWatsonTest(reg_shannon_sum_3ans)

outlierTest(reg_shannon_sum_2ans)
jarque.bera.test(reg_shannon_sum_2ans$residuals)
bptest(reg_shannon_sum_2ans)
durbinWatsonTest(reg_shannon_sum_2ans)

outlierTest(reg_shannon_sum_1an)
jarque.bera.test(reg_shannon_sum_1an$residuals)
bptest(reg_shannon_sum_1an)
durbinWatsonTest(reg_shannon_sum_1an)

#Visualisation

#Moyenne sur 2 ans
# Récupération des coefficients et du R²
summary_reg <- summary(reg_shannon_moyenne_2ans)
a <- round(coef(reg_shannon_moyenne_2ans)[2], 3)
b <- round(coef(reg_shannon_moyenne_2ans)[1], 3)
r2 <- round(summary_reg$r.squared, 3)

eq_label <- paste0("italic(y) == ", a, " * italic(x) + ", b, "~~italic(R)^2 == ", r2)

graphe1 <- ggplot(stir_aggrege_shannon, aes(x = STIR_moy_2ans, y = Shannon)) +
  geom_point(size = 2) +
  ...
  labs(
    subtitle = "STIR moyen sur 2 ans",
    x = "STIR moyen sur 2 ans",
    y = "Indice de Shannon"
  ) +
  theme_minimal() +
  theme(
    plot.margin = unit(c(1, 1, 1, 1), "cm"),
    axis.subtitle = element_text(face = "bold", color = "black", size = 14),
    axis.text = element_text(face = "bold", color = "black", size = 12),
    plot.subtitle = element_text(face = "bold", color = "black", size = 16, hjust = 0.5),
    strip.text = element_text(face = "bold", color = "black", size = 14),
    strip.background = element_rect(fill = "white", color = NA)
  )

#Somme sur 4 ans
summary_reg_4 <- summary(reg_shannon_sum_4ans)
a_4 <- round(coef(reg_shannon_sum_4ans)[2], 3)
b_4 <- round(coef(reg_shannon_sum_4ans)[1], 3)
r2_4 <- round(summary_reg_4$r.squared, 3)

eq_label_4 <- paste0("italic(y) == ", a_4, " * italic(x) + ", b_4, 
                     "~~italic(R)^2 == ", r2_4)

graphe2 <- ggplot(stir_aggrege_shannon, aes(x = STIR_sum_4ans, y = Shannon)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "blue", linewidth = 1) +
  annotate("text", x = Inf , y = -Inf, label = eq_label_4, parse = TRUE,
           hjust = 1.1, vjust = -0.5, size = 4, color = "black") +
  labs(
    subtitle = "STIR cumulé sur 4 ans",
    x = "STIR cumulé sur 4 ans",
    y = "Indice de Shannon"
  ) +
  theme_minimal() +
  theme(
...
#Somme sur 1 an
summary_reg_3 <- summary(reg_shannon_sum_1an)
a_3 <- round(coef(reg_shannon_sum_1an)[2], 3)
b_3 <- round(coef(reg_shannon_sum_1an)[1], 3)
r2_3 <- round(summary_reg_3$r.squared, 3)

eq_label_3 <- paste0("italic(y) == ", a_3, " * italic(x) + ", b_3, 
                     "~~italic(R)^2 == ", r2_3)

graphe3 <- ggplot(stir_aggrege_shannon, aes(x = STIR_sum_1an, y = Shannon)) +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "blue", linewidth = 1) +
  annotate("text", x = Inf, y = -Inf, label = eq_label_3, parse = TRUE,
           hjust = 1.1, vjust = -0.5, size = 4, color = "black") +
  labs(
    subtitle = " STIR cumulé sur 1 an",
    x = "STIR cumulé sur 1 an",
    y = "Indice de Shannon"
  ) +
  theme_minimal() +
  theme(
...
(graphe1 | graphe2 | graphe3) +
  plot_annotation(
    title = "Relations entre l’indice de Shannon et les indices de STIR",
    theme = theme(
...
stir_aggrege_simpson <- left_join(stir_aggrege, simpson_df, by = "site")

# Régressions linéaires pour l'indice de Simpson
reg_simpson_moyenne_3ans <- lm(Simpson ~ STIR_moy_3ans, data = stir_aggrege_simpson)
summary(reg_simpson_moyenne_3ans)
reg_simpson_moyenne_2ans <- lm(Simpson ~ STIR_moy_2ans, data = stir_aggrege_simpson)
summary(reg_simpson_moyenne_2ans)
reg_simpson_sum_4ans <- lm(Simpson ~ STIR_sum_4ans, data = stir_aggrege_simpson)
summary(reg_simpson_sum_4ans)
reg_simpson_sum_3ans <- lm(Simpson ~ STIR_sum_3ans, data = stir_aggrege_simpson)
summary(reg_simpson_sum_3ans)
reg_simpson_sum_2ans <- lm(Simpson ~ STIR_sum_2ans, data = stir_aggrege_simpson)
summary(reg_simpson_sum_2ans)
reg_simpson_sum_1an <- lm(Simpson ~ STIR_sum_1an, data = stir_aggrege_simpson)
summary(reg_simpson_sum_1an)

#validité du modèle
outlierTest(reg_simpson_moyenne_3ans)
jarque.bera.test(reg_simpson_moyenne_3ans$residuals)
bptest(reg_simpson_moyenne_3ans)
durbinWatsonTest(reg_simpson_moyenne_3ans)

outlierTest(reg_simpson_moyenne_2ans)
jarque.bera.test(reg_simpson_moyenne_2ans$residuals)
bptest(reg_simpson_moyenne_2ans)
durbinWatsonTest(reg_simpson_moyenne_2ans)

outlierTest(reg_simpson_sum_4ans)
jarque.bera.test(reg_simpson_sum_4ans$residuals)
bptest(reg_simpson_sum_4ans)
durbinWatsonTest(reg_simpson_sum_4ans)

outlierTest(reg_simpson_sum_3ans)
jarque.bera.test(reg_simpson_sum_3ans$residuals)
bptest(reg_simpson_sum_3ans)
durbinWatsonTest(reg_simpson_sum_3ans)

outlierTest(reg_simpson_sum_2ans)
jarque.bera.test(reg_simpson_sum_2ans$residuals)
bptest(reg_simpson_sum_2ans)
durbinWatsonTest(reg_simpson_sum_2ans)

outlierTest(reg_simpson_sum_1an)
jarque.bera.test(reg_simpson_sum_1an$residuals)
bptest(reg_simpson_sum_1an)
durbinWatsonTest(reg_simpson_sum_1an)
...
stir_aggrege_pielou <- left_join(stir_aggrege, pielou_df, by = "site")

# Régressions linéaires pour l'indice de Piélou
reg_pielou_moyenne_3ans <- lm(Pielou ~ STIR_moy_3ans, data = stir_aggrege_pielou)
summary(reg_pielou_moyenne_3ans)
reg_pielou_moyenne_2ans <- lm(Pielou ~ STIR_moy_2ans, data = stir_aggrege_pielou)
summary(reg_pielou_moyenne_2ans)
reg_pielou_sum_4ans <- lm(Pielou ~ STIR_sum_4ans, data = stir_aggrege_pielou)
summary(reg_pielou_sum_4ans)
reg_pielou_sum_3ans <- lm(Pielou ~ STIR_sum_3ans, data = stir_aggrege_pielou)
summary(reg_pielou_sum_3ans)
reg_pielou_sum_2ans <- lm(Pielou ~ STIR_sum_2ans, data = stir_aggrege_pielou)
summary(reg_pielou_sum_2ans)
reg_pielou_sum_1an <- lm(Pielou ~ STIR_sum_1an, data = stir_aggrege_pielou)
summary(reg_pielou_sum_1an)

#validité du modèle
outlierTest(reg_pielou_moyenne_2ans)
jarque.bera.test(reg_pielou_moyenne_2ans$residuals)
bptest(reg_pielou_moyenne_2ans)
durbinWatsonTest(reg_pielou_moyenne_2ans)

outlierTest(reg_pielou_sum_3ans)
jarque.bera.test(reg_pielou_sum_3ans$residuals)
bptest(reg_pielou_sum_3ans)
durbinWatsonTest(reg_pielou_sum_3ans)
