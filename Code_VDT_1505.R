...
#_______________________________________________________________________________________
Liste=read_excel("C:\\R_projects\\DB_F4F_2025_Antoine110425.xlsx",sheet="Liste_Sites")
Coord = read_excel("C:\\R_projects\\DB_F4F_2025_Antoine110425.xlsx", sheet = "Coords")
Dates = read_excel("C:\\R_projects\\DB_F4F_2025_Antoine110425.xlsx", sheet = "Dates")
Sites = Liste[, c(3:6)]
VDT=read_excel("C:\\R_projects\\DB_F4F_2025_Antoine110425.xlsx",sheet="VDT")
Weight_VDT=read_excel("C:\\R_projects\\DB_F4F_2025_Antoine110425.xlsx",sheet="Weight_VDT")
...
#--------------5. Diversité alpha et indices associés-------------------
...
#---------------5.5 Visualisation----------------------
#Shannon
ggplot(shannon_df, aes(x = TYPE, y = Shannon, fill = TYPE)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +  # Suppression des outliers pour éviter le fouillis
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") + # Ajout des points pour mieux voir la distribution
  scale_fill_manual(values = palette_colors) +
  labs(title = "Indice de Shannon par pratique agricole",
       x = "Pratique agricole",
       y = "Indice de Shannon") +
  theme_bw() +
  theme(
...
#Simpson
ggplot(simpson_df, aes(x = TYPE, y = Simpson, fill = TYPE)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +  # Suppression des outliers pour éviter le fouillis
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") + # Ajout des points pour mieux voir la distribution
  scale_fill_manual(values = palette_colors) +
  labs(title = "Indice de Simpson par pratique agricole",
       x = "Pratique agricole",
       y = "Indice de Simpson") +
  theme_bw() +
  theme(
...
#Piélou
ggplot(pielou_df, aes(x = TYPE, y = Pielou, fill = TYPE)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +  # Suppression des outliers pour éviter le fouillis
  geom_jitter(width = 0.2, alpha = 0.5, color = "black") + # Ajout des points pour mieux voir la distribution
  scale_fill_manual(values = palette_colors) +
  labs(title = "Indice de Piélou par pratique agricole",
       x = "Pratique agricole",
       y = "Indice de Piélou") +
  theme_bw() +
  theme(
...
#_______________________________________________________________________________________
#Calculer les indices de Hill-Shannon, Hill-Simpson et Piélou
#Faire la même chose que pour la biomasse, l'abondance et le nombre d'espèces
#(ANOVA/Kruskal-Wallis + tests + Visualisation en boxplots)
